...
<div class="card border-0 shadow-sm rounded-4">
    <div class="card-body">
        ...
        <form action="{{ route('admin.buku.update', $buku->id) }}" method="POST" class="row g-3">
            @csrf
            @method('PUT')
            ...
            <div class="col-md-4">
                ...
                <input type="text" id="tahun_terbit" name="tahun_terbit" class="form-control"
                    value="{{ old('tahun_terbit', $buku->tahun_terbit) }}" placeholder="2024">
            </div>

            <div class="col-md-4">
                <label for="bahasa" class="form-label">Bahasa</label>
                <input type="text" id="bahasa" name="bahasa" class="form-control"
                    value="{{ old('bahasa', $buku->bahasa) }}" placeholder="Indonesia">
            </div>

            <div class="col-md-4">
                <label for="isbn" class="form-label">ISBN</label>
                <input type="text" id="isbn" name="isbn" class="form-control"
                    value="{{ old('isbn', $buku->isbn) }}" placeholder="978-602-1234-5">
            </div>

            <div class="col-md-6">
                <label for="jumlah_stok" class="form-label">Jumlah Stok</label>
                <input type="number" id="jumlah_stok" name="jumlah_stok" class="form-control" min="0"
                    value="{{ old('jumlah_stok', $buku->jumlah_stok) }}" required>
            </div>

            <div class="col-12">
                <label for="deskripsi" class="form-label">Deskripsi / Sinopsis</label>
                <textarea name="deskripsi" id="deskripsi" rows="3" class="form-control">{{ old('deskripsi', $buku->deskripsi) }}</textarea>
            </div>
            ...
            <div class="col-12">
                ...
                <select name="gambar_buku_id" id="gambar_buku_id" class="form-select d-none" aria-hidden="true">
                    <option value="" {{ old('gambar_buku_id', $buku->gambar_buku_id) ? '' : 'selected' }}>Pilih file</option>
                    @foreach ($files as $file)
                        ...
                        <option value="{{ $file->id }}" data-preview="{{ $previewPath }}" data-name="{{ $fileName }}"
                            {{ (string) old('gambar_buku_id', $buku->gambar_buku_id) === (string) $file->id ? 'selected' : '' }}>
                            {{ $fileName }}
                        </option>
                    @endforeach
                </select>
...
document.addEventListener('DOMContentLoaded', () => {
    // Sama seperti create, gunakan logika yang sama
    ...
    @if(old('gambar_buku_id', $buku->gambar_buku_id))
        fileSelect.value = '{{ old('gambar_buku_id', $buku->gambar_buku_id) }}';
        updateFilePreview();
    @endif
});
</script>
